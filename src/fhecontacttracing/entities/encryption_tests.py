...
class EncryptionMOTest(unittest.TestCase):
    ...
    def test_datatomo(self):
        ...
        self.assertLessEqual(abs(deco_score[0].real - 6), 1 / 156, "send data to mo no work")
...
class EncryptionSTLTest(unittest.TestCase):
    ...
    def test_tick(self):
        ...
        test_stl = EncryptionSTL(movements_iterable=interiter,
                                 mo_count=2,
                                 risk_thr=5,
                                 area_sizes=(50, 50),
                                 max_sizes=(3652, 3652),
                                 ...
                                 big_modulus=1 << 1200,
                                 scaling_factor=1 << 30)
        test_stl._curr_time = 1  # if you don't do this, status get refreshed to 0 after first tick
        ...
        for itercount in range(10):
            ...
            for mo in test_stl._mos:
                for i in range(len(mo._scores)):
                    ...
                    self.assertLessEqual(abs(deco_score[0].real - 9 * (itercount + 1)), 1 / 156,
        ...
        test_stl = EncryptionSTL(movements_iterable=sameloc,
                                 ...
                                 big_modulus=1 << 1200,
                                 scaling_factor=1 << 30)
        test_stl._curr_time = 1  # unless you do this, status will refresh to 0 after first tick
        ...
        for itercount in range(2):
            ...
            for mo in test_stl._mos[1:]:
                for i in range(len(mo._scores)):
                    decr_score = mo._GA._decryptor.decrypt(ciphertext=mo._scores[i])
                    deco_score = mo._encoder.decode(plain=decr_score)
                    self.assertLessEqual(abs(deco_score[0].real - (itercount + 1)), 1 / 156, "outside scoring no good")
        ...
        for i in range(len(test_stl._ga._scores)):
            self.assertLessEqual(abs(test_stl._ga._scores[i] - i ** 2), 1 / 156,
        ...
        for i in range(len(test_stl._mos)):
            for j in range(len(test_stl._mos[i]._status)):
                ...
                self.assertLessEqual(abs(val - test_stl._mos[i]._users[j]._uID), 7.1e-9,
                                     "status not properly updated in MO " + str(i) + " for user " + str(
                                         test_stl._mos[i]._users[j]._uID))
        ...
        for itercount in range(10):
            ...
            if itercount == 0:  # only check GA equalities after daily
                for i in range(100):
                    self.assertLessEqual(abs(test_stl._ga._scores[i] - plain_stl._ga._scores[i]), 1 / 156,
            ...
            for i in range(test_stl._mo_count):
                for j in range(len(test_stl._mos[i]._users)):
                    ...
                    self.assertEqual(test_stl._mos[i]._users[j]._x, plain_stl._mos[i]._users[j]._x,
                                     "not same x coord in MO at " + str(test_stl._mos[i]._users[j]._uID))
                    self.assertEqual(test_stl._mos[i]._users[j]._y, plain_stl._mos[i]._users[j]._y,
                                     "not same y coord in MO at " + str(test_stl._mos[i]._users[j]._uID))
                    decr = test_stl._ga._decryptor.decrypt(ciphertext=test_stl._mos[i]._scores[j])
                    deco = test_stl._mos[i]._encoder.decode(plain=decr)
                    val = deco[0].real
                    self.assertLessEqual(abs(val - plain_stl._mos[i]._scores[j]), 1 / 156,
                                         "score not same in MO at " + str(test_stl._mos[i]._users[j]._uID))
